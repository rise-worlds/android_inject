...
//         isAllowed = true;
//         // // 使用资格验证
//         // if (!isAllowed && connectCount <= 15 && !AsobimoId.empty())
//         // { // 还没有进行授权验证过 或者授权验证已经10+次 并且游戏ID不为空
//         // 	connectCount++;
...
//         // if (myGameHack.isEnableCheckBox1 && isAllowed)
//         // { // 技能全屏
...
//         // if (myGameHack.isEnableCheckBox2 && isAllowed)
//         // { // 定怪
...
//         // if (myGameHack.isEnableCheckBox3 && isAllowed)
//         // { // 开启了无敌按钮 并且 授权验证成功
...
//         // if (myGameHack.isEnableCheckBox4 && isAllowed)
//         // { // 开启了无敌按钮 并且 授权验证成功
...
// 实现hook不崩溃
EGLBoolean (*origin_EglSwapBuffers)(EGLDisplay dpy, EGLSurface surface);
EGLBoolean my_EglSwapBuffers(EGLDisplay dpy, EGLSurface surface)
{
    ...
    // 原始函数调用
    return origin_EglSwapBuffers(dpy, surface);
}

// // 触摸设置
// void (*origin_Input)(void *thiz, void *a2, void *a3);
// void my_Input(void *thiz, void *a2, void *a3)
// {
// 	// 原始函数调用
...
void *(*origin__loader_dlopen)(const char *filename, int flags, const void *caller_addr);
void *my__loader_dlopen(const char *filename, int flags, const void *caller_addr)
{
    void *handle = origin__loader_dlopen(filename, flags, caller_addr);
    dlopen_process(filename, handle);
    return handle;
}

void *(*origin_do_dlopen_V24)(const char *name, int flags, const void *extinfo, void *caller_addr);
void *my_do_dlopen_V24(const char *name, int flags, const void *extinfo, void *caller_addr)
{
    void *handle = origin_do_dlopen_V24(name, flags, extinfo, caller_addr);
    dlopen_process(name, handle);
    return handle;
}

void *(*origin_do_dlopen_V19)(const char *name, int flags, const void *extinfo);
void *my_do_dlopen_V19(const char *name, int flags, const void *extinfo)
{
    void *handle = origin_do_dlopen_V19(name, flags, extinfo);
    dlopen_process(name, handle);
    return handle;
}
...
void *main_thread(void *)
{
    ...
    else if (apiLevel >= 24)    // android 7.0 虚拟机适配
    {
        ...
        if (addr)
        {
            DobbyHook(addr, (void *)my_do_dlopen_V24, (void **)&origin_do_dlopen_V24);
        }
    }
    else
    {
        ...
        if (addr)
        {
            DobbyHook(addr, (void *)my_do_dlopen_V19, (void **)&origin_do_dlopen_V19);
        }
    }
    ...
    // TODO:优化dlopen处理
    while (il2cppAddress == 0 || il2cppHandle == nullptr)
    { // 动态库已经完全加载
    ...
    {
        // 打印eglSwapBuffers地址
        LOGD("eglSwapBuffers is %p", sym_eglSwapBuffers);
         // 启动hook
        myHook((unsigned long)sym_eglSwapBuffers, (void *)my_EglSwapBuffers, (void **)&origin_EglSwapBuffers, "eglSwapBuffers");
    }
    ...
    //     myHook((unsigned long)sym_input, (void *)my_Input, (void **)&origin_Input, "_ZN7android13InputConsumer21initializeMotionEventEPNS_11MotionEventEPKNS_12InputMessageE"); // 创建hook
    // }
